...
```{r}
plot(bind_patternogram(p_train_pred, p_pr))
```
...
```{r}
plot(bind_patternogram(p_train_pred2, p_pr2))
```

```{r}
library(ggplot2)
autoplot(bind_patternogram(p_train_pred2, p_pr2)) + 
  facet_wrap(~group)
```
...
p_12 = bind_patternogram(p1, p2, ids = c("unweighted", "weighted"))
plot(p_12)
```
...
plot(bind_patternogram(p_temp_train, p_prediction, ids = c("stations", "prediction")))
```
